...
- 2026-05-09: REQ-017 Act 2 second loop integration lands (REQ-017 done; section 4 REQ-017 dossier slice). Tests-only slice. New file `tests/sim/act2Loop2.test.ts` (2 cases) drives the existing primitives through the GDD's scripted second-loop sequence and asserts the `ActStateObserver` transitions to `act-2-loop-2`. Phase 1+2: a `runLoopOne` helper replays the loop-1 sequence (East at 5:00 then West at 6:00) leaving ghost-A in bucket 5, ghost-B in bucket 6, active timeline 5, and ghost-A replayed to exhaustion; the observer is opened mid-test and fed the loop-1 snapshot so its watermark advances to `act-2-loop-1` (the per-beat slices wire the host's observer once per fixed step in production; this test mirrors that monotonic advance to pin the chain prerequisite walk). Phase 3a: the active player is positioned next to ghost-A and the same side effects the host's punch resolver applies fire (`applyKnockout(ghostA.consciousness)` flips it to `'unconscious'`; `applyKnockoutBodyResponse(ghostA.body, ghostA.mesh, direction)` writes the bump impulse plus the mesh tilt). Phase 3b: `resolveCarryToggle` with a one-tick rising edge of pickup picks up ghost-A; the resulting `CarryState` is `{ kind: 'carrying', carriedId: ghostA.instanceId }`; `applyCarryPickup` flips ghost-A's body to `KinematicPositionBased`. Phase 3c: four input frames are recorded (`right: true, pickup: true`); `applyCarryAttachment` runs per recorded frame the way the host loop does; `detector.step` then crosses the East trigger at 5:00 (East lit per `DOOR_STATE_BY_HOUR`); the lit-portal traversal handler files the carrying-East lifetime as a fresh ghost in bucket 5 (so bucket 5 ends with at least 2 ghosts: the original unconscious ghost-A plus the dragging-East ghost-C, both hidden), switches the active timeline to 6, and `player.carry` passes through unchanged because `wireTraversal` does not branch on the carry state per `ActivePlayerHandle.carry` docstring. Phase 3d: post-traversal assertions: active=6, `player.carry` preserved, bucket 5 has at least one unconscious ghost (ghost-A retained its flag because `setActiveTimeline` only HIDES the leaving bucket; `reset()` only fires on the entering bucket's re-entry path, not on leaving), bucket 6 still holds ghost-B from the prior loop. Phase 4 resolves the GDD's 'wait for another instance to wake up' beat per Q-023 default A: under REQ-033 done, unconsciousness is permanent in v1 (the dossier explicitly forbids the unconscious-to-conscious transition: "There is no recovery in v1: once unconscious, the capsule stays tilted until pickup or hard reset"), so the literal wake mechanic cannot be staged inside one play session. Q-023 default A treats 'wake up' as the recorded punch tick firing: the GDD beat is not actually about the body waking but about another instance's recorded sequence playing back a punch against You1 at 6:00. The integration test models the outcome (You1 goes down at 6:00) by directly applying `applyKnockout` plus `applyKnockoutBodyResponse` against the active player, which is exactly what the recorded punch tick would do on the next loop iteration. The recording substrate of REQ-001 / REQ-002 satisfies the literal version on any future visit to 6:00 (the recording made during this test will replay the punch). Phase 5 builds the final `ActStateSnapshot`, asserts `isAct2Loop2(snapshot) === true`, and asserts the observer (already at `act-2-loop-1` watermark) transitions to `act-2-loop-2` via `observer.update(snapshot)`. A second case pins the four conjunctions of `isAct2Loop2` as load-bearing: it builds the full satisfying state and then drops each conjunction in turn (currentTimeline switched to 5 via a synthesized snapshot, active player flipped conscious, no unconscious ghost in bucket 5 by mapping the ghosts5 projection to `'conscious'`, empty bucket 6 by returning `[]` for that timeline) and asserts `isAct2Loop2` returns false on each variant. NO production code touched: the existing `isAct2Loop2` predicate from PR #30 fires correctly on the live-state snapshot, confirming `currentTimeline === 6 && activePlayer.consciousness === 'unconscious' && ghosts5.some(unconscious) && ghosts6.length >= 1` is the correct dossier-pseudocode mapping. With REQ-017 done, the next slice (REQ-018) wires the Act 3 setup beat. Files: `tests/sim/act2Loop2.test.ts`, `docs/gdd/03-story-acts-1-3.md`, `docs/gdd/40-act-progress-and-narrative-beats.md`, `docs/PROGRESS_LOG.md`, `docs/GDD_COVERAGE.json`, `docs/OPEN_QUESTIONS.md` (Q-023 added). PR pending.
- 2026-05-09: REQ-016 Act 2 first loop integration lands (REQ-016 done; section 4 REQ-016 dossier slice). Tests-only slice. New file `tests/sim/act2Loop1.test.ts` (2 cases) drives the existing primitives through the scripted East-then-West sequence and asserts the `ActStateObserver` transitions to `act-2-loop-1`. Phase 1: the active player records a 5-frame East-bound walk at 5:00 and crosses the East portal trigger; the traversal handler files the You1 ghost into bucket 5 (instance 1, recording length 5, hidden because timeline 5 is no longer active) and switches the active timeline to 6 with a fresh recorder. Phase 2: the active player records a 4-frame West-bound walk at 6:00 and crosses the West portal trigger; the traversal handler files the You-1 ghost into bucket 6 (instance 2, recording length 4, hidden) and switches the active timeline back to 5. The 5:00-recorded ghost is reset to tick 0 and shown via the registry's entering-bucket reset path. Phase 3: `advanceTick` is driven for the full length of ghost-A's recording so `tickIndex >= recordingLength`, satisfying `allGhostsAtRest`. Phase 4: a snapshot is projected from the live registry plus the active player (the same `BucketGhostSnapshot` shape `mountAct1Cinematic`'s integration test built in PR #34), `isAct2Loop1(snapshot)` reads true, and `observer.update(snapshot)` returns `'act-2-loop-1'` after walking the chain from `not-started` through `act-1-spawn` (cinematic in 12:00 from `mountAct1Cinematic` at boot). A second case pins ghost-A's per-tick replay velocity to be byte-identical to `inputToVelocity(recording.frames[i].keys)` for every tick within the recording, backing the dossier's 'and disappears' semantics with a determinism guard. NO production code touched: the `isAct2Loop1` predicate from PR #30 fires correctly on the live-state snapshot, confirming the predicate body is the correct read of the dossier's pseudocode in section 4 REQ-016. With REQ-016 done, the next slice (REQ-017) wires the second-loop beat. Files: `tests/sim/act2Loop1.test.ts`, `docs/gdd/03-story-acts-1-3.md`, `docs/gdd/40-act-progress-and-narrative-beats.md`, `docs/PROGRESS_LOG.md`, `docs/GDD_COVERAGE.json`. PR pending.
- 2026-05-09: REQ-012 Act 1 cinematic via scripted actor recordings lands (REQ-012 done; section 5 dossier slice). New module `src/sim/scripts/act1Cinematic.ts` exports the three frozen `InputRecording` objects (`ACT1_LEFT_DRAGGER_RECORDING` and `ACT1_RIGHT_DRAGGER_RECORDING` at 240 frames with `back: true, pickup: true` held for the first 60 ticks then idle for 180 ticks; `ACT1_KNOCKOUT_BODY_RECORDING` as a 1-frame all-zero ghost per Q-021 default A), the spawn poses (`ACT1_LEFT_DRAGGER_SPAWN = { x: -0.6, z: -3.5 }`, `ACT1_RIGHT_DRAGGER_SPAWN = { x: 0.6, z: -3.5 }`, `ACT1_KNOCKOUT_BODY_SPAWN = { x: 0, z: -3.6 }`), the timing constants (`ACT1_CINEMATIC_DURATION_TICKS = 240`, `ACT1_CINEMATIC_FADE_START_TICK = 180`, `ACT1_CINEMATIC_FADE_DURATION_TICKS = 60`, `ACT1_DRAGGER_WALK_TICKS = 60`), and the host hook `mountAct1Cinematic({ registry, scene, world })` which builds three `GhostInstance` objects via `createGhost` (instance ids 101, 102, 103) and files them into the 12:00 bucket via `registry.add(12, ghost)`, flipping the body ghost to `unconscious` post-creation via `applyKnockout`. New module `src/render/fadeOverlay.ts` exports `createFadeOverlay({ colorHex? })` per Q-013 default A: a Three.js full-screen `PlaneGeometry(2, 2)` on a separate `OrthographicCamera` with a `MeshBasicMaterial` (`transparent: true, depthTest: false, depthWrite: false`), `setOpacity(t)` (clamped to `[0, 1]`, hides the mesh at zero so the renderer skips the draw call), `render(renderer)` (composites over the main pass via `renderer.autoClear = false`), and `dispose()` (frees geometry / material and removes the mesh from its scene). `src/app.ts` mounts the cinematic at boot right after the registry is created (active timeline at boot is 5:00 so the cinematic ghosts open hidden via the registry's `add` non-active-bucket hide path) and constructs the fade overlay whose `render` runs after the main `renderer.render` each frame. The overlay opens at opacity 0 and stays cost-free until a future cinematic-timing slice writes through `setOpacity`; this slice ships the primitive plus the call site. Q-012 / Q-013 / Q-015 / Q-016 / Q-021 defaults are all consumed. The visible body-tracking-the-actors behavior is OUT of scope this slice: the host loop's carry resolver only fires for the active player, so the body sits at its spawn pose while the actors walk past it; a future slice can extend the carry attachment to fire for ghost-to-ghost pickups. With REQ-012 done, the `isAct1Spawn` predicate (which reads `ghostsFor(12).length >= 3 && currentTimeline === 5`) is now satisfied at boot, so when the per-beat slices wire the observer into `src/app.ts` the watermark will advance to `act-1-spawn` on the first tick after boot. Files: `src/sim/scripts/act1Cinematic.ts`, `src/render/fadeOverlay.ts`, `src/app.ts`, `tests/sim/scripts/act1Cinematic.test.ts`, `tests/render/fadeOverlay.test.ts`, `docs/gdd/03-story-acts-1-3.md`, `docs/gdd/40-act-progress-and-narrative-beats.md`, `docs/PROGRESS_LOG.md`, `docs/GDD_COVERAGE.json`. PR pending.
- 2026-05-09: REQ-004 no-paradox property tests land (REQ-004 done; section 7 dossier slice). Tests-only slice. New file `tests/sim/noParadox.test.ts` codifies the four invariants from section 7 as property-style tests, each running at least 100 randomized input sequences derived from a hand-rolled 32-bit Lehmer LCG (Park-Miller multiplier 48271 mod 0x7fffffff) seeded from the test name via a small djb2 hash (Q-017 default). The LCG is a 30-line implementation with no third-party dependency (Rule 3 stack constraint). Invariant 1 (recording immutability post-snapshot): asserts `Object.isFrozen` at every level of the snapshot, asserts further recording into the parent recorder leaves the snapshot's observable state unchanged, and asserts explicit mutation attempts via `frames.push`, `frame.tick = -1`, `frame.keys.forward = !forward`, and `snap.length = 0` are silently rejected or throw under strict mode and leave the snapshot observably unchanged in deep equality. Invariant 2 (registry monotonicity): runs 50 random operations per trial against `createTimelineRegistry({ initialTimeline: 5 })`, mixing 70% `add` (random timeline in {5, 6, 12}) and 30% `setActiveTimeline`, asserts the cross-bucket sum is monotonically non-decreasing across every operation, then asserts `clearAllGhosts(scene, world, 5)` returns the count to zero. Invariant 3 (tick-K causality): records a sequence S of length k = 15+rand(30), snapshots at K, builds ghost A from the K-snapshot in a fresh world and runs 20 fixed simulation steps capturing each per-tick body translation, then continues recording into the SAME recorder for 10+rand(80) additional random ticks, builds ghost B from the K-snapshot in another fresh world, and asserts ghost B's per-tick body translations are bit-identical to ghost A's (`expect(tr.x).toBe(a.x)` rather than `toBeCloseTo`, because the same recording in the same fixed-step world from the same start position is deterministic to the bit). A second invariant 3 case asserts `replayAtTick(snap, t)` reads the same value before and after the recorder is mutated past K. Section 7 invariant 4 (Portal.destinationHours readonly across a 1000-tick simulation) was already discharged by the REQ-005 polish flip (PR #31) which lands the property test in `tests/sim/portal.test.ts`; this slice adds a complementary cross-cutting case in `tests/sim/noParadox.test.ts` that asserts a snapshot's per-frame keys remain equality-stable after the recording is filed into a registry and the ghost runs five fixed simulation steps, so the no-paradox surface is verified end-to-end across both the portal data model (REQ-005's slice) and the recorder / registry / ghost surface (this slice). NO production code touched; the existing `InputRecorder` / `TimelineRegistry` / `GhostInstance` / `replayAtTick` implementations already satisfy every invariant. With REQ-004 done and REQ-005 done, section 7's Pillar 2 invariants are fully discharged. Files: `tests/sim/noParadox.test.ts`, `docs/gdd/02-time-travel-rules.md`, `docs/gdd/40-act-progress-and-narrative-beats.md`, `docs/PROGRESS_LOG.md`, `docs/GDD_COVERAGE.json`. PR pending.
