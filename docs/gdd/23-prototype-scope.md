...
- 2026-05-08: REQ-029 done. `TimeOfDay` is now locked to the deterministic simulation tick. Internal state is an integer `tickIndex` modulo `ticksPerCycle` (default 3600 = 60s * 60Hz). New API `advanceTicks(n)` plus `tick()` and a convenience `advanceSeconds(dt)` that rounds. The fixed-step physics loop in `src/app.ts` calls `timeOfDay.advanceTicks(1)` once per step instead of `timeOfDay.advance(deltaMs / 1000)` per render frame, so the same fixed-step count always produces the same normalized output regardless of frame rate. Tests cover the determinism contract (`advanceTicks(N)` equals `N x advanceTicks(1)`, no drift after 1000 cycles, integer-only deltas). Files: `src/sim/timeOfDay.ts`, `src/app.ts`, `tests/sim/timeOfDay.test.ts`. PR (pending).
