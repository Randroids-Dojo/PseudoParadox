...
### Persistence across loops

A throw at 5:00 lands a body at 12:00. The player walks east to 6:00, loops back to 5:00, and their prior 5:00 recording replays. The recorded throw input fires again at the recorded tick. The implementation question this raises is: should a fresh thrown body spawn on every replay (producing a duplicate at the destination each loop), or should the throw be a one-shot keyed to the throw event so subsequent loops see the same single body persist?

Per Q-028 the resolution is **one body per recorded throw event, across all loops**. Concretely:

- Each thrown body receives a unique `bodyId` at throw time. The id is the tuple `(throwerInstanceId, throwTick)` derived from the carrier's lifetime: deterministic across replays because both fields are recorded inputs that survive snapshot.
- An `InFlightRegistry` (the existing one from REQ-036 plus the F-007 implementation slice) tracks every thrown body by `bodyId` across timeline buckets.
- On every replay of the throw input the throw resolver looks up `bodyId` in the registry. If a body with that id exists ANYWHERE (any timeline bucket, in-flight or settled), the resolver no-ops: no fresh spawn, no impulse, no body. If no body with that id exists (the canonical "first throw" case), the resolver spawns a fresh body and applies the impulse as before. The first traversal of the throw input is therefore the canonical body; every subsequent replay is silent at the recorded tick.
- On lit-portal traversal, the body's bookkeeping is REHOMED: removed from the source-timeline bucket and added to the destination-timeline bucket. The Rapier rigid body stays alive (same `RigidBodyHandle`); only the registry's bucket-by-timeline index updates. This is the IMPLEMENTATION work F-007 calls out: today the body translation teleports across the portal but the in-flight bookkeeping stays in the source bucket, so on a future return to the source the registry's reset path snaps the body back to its source spawn pose. After F-007, the source bucket no longer holds a reference to the thrown body once it has crossed the portal; the destination bucket does, so the player only sees the body when they visit the destination timeline.
- Pickup of a thrown body re-uses the same `bodyId`. If the body is picked up at 12:00 (after a throw from 5:00) and carried back through a portal to 5:00, the pickup-carry-traverse path rehomes the body again per the carry's source-timeline tracking. The id never changes; the bookkeeping follows where the body lives.
- Hard reset clears the in-flight registry alongside every other timeline bucket, so a fresh `bodyId` namespace opens on every Act 1 reset.

UX consequence: on a loop-back visit to the source timeline (e.g. returning to 5:00 after the throw originally fired there), the player does NOT see a phantom arc replay of the throw. The body lives in the destination timeline; the source timeline's recording is replayed exactly as before (REQ-002 frame-exact replay applies to inputs, not to side-effecting body spawns that have already produced a persistent body). This is intentional: it matches the "no paradoxes" pillar by ensuring a single physical body per recorded throw event regardless of how many times the player loops.
...
- 2026-05-11: F-007 dossier amendment landed (section 7 "Persistence across loops" subsection added; Q-028 resolved on birth). Specifies the "one body per recorded throw event across all loops" model: each thrown body carries a `bodyId = (throwerInstanceId, throwTick)`, the in-flight registry tracks bodies by id across timeline buckets, on lit-portal traversal the body is rehomed from source to destination bucket (no re-spawn), and on replay the throw resolver no-ops when a body with the same id already exists anywhere. UX consequence: no phantom-arc replay at the source timeline on loop-back; the body lives in the destination timeline only after first traversal. Spec-only; the implementation lands in a follow-up F-007 slice. Files: `docs/gdd/30-combat-and-interaction.md`, `docs/OPEN_QUESTIONS.md` (Q-028 added), `docs/FOLLOWUPS.md` (F-007 unblock condition met). PR pending.
- 2026-05-09: Thought-bubble icon overlay landed (REQ-032 done; section 10 slice 6). With this slice all five combat REQs (REQ-032 through REQ-036) are landed; the section's `Status:` flips from `partial` to `done`. New module `src/render/thoughtBubble.ts` exports `createThoughtBubble(scene)` returning a `ThoughtBubble` whose `group` is a parented `THREE.Group` with a disc backplate plus per-icon child meshes (`door`, `fist`, `sleep`, `pickup`, `throw`), the constants `THOUGHT_BUBBLE_Y_OFFSET = PLAYER_CAPSULE_TOTAL_HEIGHT / 2 + 0.6` and `THOUGHT_BUBBLE_RADIUS = 0.22`, and the `setIcon` / `update` / `dispose` methods. The disc and the per-icon glyphs are generated programmatically (door = arrow shape; fist = filled circle plus a smaller centered circle; sleep = three plane-rectangles arranged in a Z; pickup = upward arrow; throw = parabolic arc from twelve plane segments). All glyphs share a `MeshBasicMaterial` with `depthTest: false` and a high `renderOrder` so the bubble draws on top of room geometry. New module `src/sim/thoughtBubblePeek.ts` exports the pure `nextQualitativelyDifferentAction(options)` helper plus `DEFAULT_LOOKAHEAD_TICKS = 30` (Q-010 resolved to A) and the `ThoughtPeekKind` union (`'sleep' | 'throw' | 'pickup' | 'fist' | 'door'`). The function returns `'sleep'` immediately if `isCurrentlyUnconscious` is true, otherwise scans ticks `currentTick + 1 .. currentTick + lookaheadTicks` for the FIRST priority match across `throw > pickup > fist > door`. Door-entry detection forward-integrates the recorded planar velocity at the fixed timestep against the supplied `PortalTrigger` array; the same `pointInsideTrigger` predicate the active player's detector uses. Walking is the baseline; the function never returns a `'footsteps'` kind (anti-spam by omission). `GhostInstance` gains a `thoughtBubble` field; `createGhost` constructs one bubble per ghost. `GhostInstance.reset()` hides the bubble. The host loop in `src/app.ts` runs the lookahead and bubble update once per render frame (NOT per fixed-step tick) and calls `setIcon` only when the kind has changed. `TimelineRegistry.clearAllGhosts` calls `bubble.dispose()` per ghost during teardown; `setActiveTimeline`'s hide pass calls `bubble.setIcon(null)`. Active player has NO bubble (only ghosts carry previews per the dossier). Files: `src/render/thoughtBubble.ts`, `src/sim/thoughtBubblePeek.ts`, `src/sim/ghostInstance.ts`, `src/sim/timelineRegistry.ts`, `src/app.ts`, `tests/render/thoughtBubble.test.ts`, `tests/sim/thoughtBubblePeek.test.ts`, `tests/sim/ghostInstance.test.ts`, `tests/sim/hardReset.test.ts`. PR #N.

- 2026-05-08: Throw with portal traversal for ballistic bodies landed (REQ-036 done; section 10 slice 5). New module `src/sim/throw.ts` exports `THROW_IMPULSE_N = 14` (planar facing impulse), `THROW_UP_IMPULSE_N = 4` (vertical bump for an arc), the pure helper `computeThrowImpulse(facing)`, the side-effecting `applyThrow(body, facing)` (Dynamic flip + zero linvel + impulse), and the high-level transition `tryThrow(opts)` that returns `{ kind: 'idle' }` on a successful throw or the input carry state on any precondition miss (no rising edge, not carrying, body resolver returned null). New module `src/sim/facing.ts` exports `DEFAULT_FACING = { x: 0, z: -1 }` (north, Q-007 default), `facingFromVelocity(v)` (normalizes non-zero velocities, returns null below epsilon), and `createFacingTracker()` whose `update(velocity)` caches the last non-zero facing across calls. New module `src/sim/bodyTraversal.ts` exports `IN_FLIGHT_SETTLE_SPEED_SQ = 0.09`, `IN_FLIGHT_SETTLE_TICKS = 2`, the `InFlightBodyHandle` and `BodyLitGate` types, and `createInFlightRegistry({ triggers })` which exposes `register`, `inFlight`, `step(isLit, resolveSpawnPose?)`, and `clear(scene, world)`. On a lit-portal `enter` for a tracked body, the registry teleports the body to the destination spawn pose with linvel preserved (Q-008 default: linvel rotated zero degrees) and clears its overlap flags so the post-teleport pose at the room center does not fire stale entries. A body whose total velocity drops below the settle threshold for `IN_FLIGHT_SETTLE_TICKS` consecutive ticks unregisters; settled bodies do not re-traverse. `KeyState` gained a `throw: boolean` channel (`KeyT` default per Q-002). `replayThrowAtTick` is the sibling to `replayPunchAtTick` / `replayPickupAtTick`. The host loop in `src/app.ts` derives the throw rising edge from `keyboard.state.throw && !previousThrowHeld && consciousness === 'conscious'`, runs `tryThrow` after the toggle resolver, suppresses the dropping snap helper when the transition was a throw (the impulse already moved the body), and registers the thrown ghost body in the in-flight registry. The facing tracker is updated each fixed step from the player's intended planar velocity (after carry speed scaling) so a recorded `KeyState` sequence yields a deterministic facing trajectory on replay. The in-flight registry is stepped after `world.step()` and `portalTriggers.step` so its detector reads the post-integration translation; the lit gate it consumes mirrors the player's traversal lit gate (`litStateForTimeline` first, falling back to authored `isLit`). `hardReset` gains optional `inFlightRegistry` and `facingTracker` fields and clears both during the reset sequence. Determinism (Q-009 default trust Rapier deterministic step) is pinned by `tests/sim/throwDeterminism.test.ts`: two identical world setups with identical pre-throw bodies and identical impulse along the same facing produce identical final positions after 30 ticks. Thrown bodies do NOT spawn ghosts (closed-form decision). Files: `src/sim/throw.ts`, `src/sim/facing.ts`, `src/sim/bodyTraversal.ts`, `src/input/keyboard.ts`, `src/sim/inputRecorder.ts`, `src/sim/hardReset.ts`, `src/app.ts`, `tests/sim/throw.test.ts`, `tests/sim/facing.test.ts`, `tests/sim/bodyTraversal.test.ts`, `tests/sim/throwDeterminism.test.ts`, `tests/sim/inputRecorder.test.ts`. PR #N.

- 2026-05-08: Drag regression tests landed (REQ-035 done; section 10 slice 4). Tests-only slice pinning the drag invariants without touching production code. New file `tests/sim/dragRegression.test.ts` covers nine cases across four describes: body tracks carrier across a 5-tick walk (deterministic one-step lag from `setNextKinematicTranslation`); body tracks carrier through a direction change; zero-velocity ticks freeze the carried body's planar position relative to the carrier within a 0.5m tolerance (absorbing the documented Q-006 followup where kinematic-vs-dynamic contact produces a small kick); drop mid-walk leaves the body at the carrier's current planar pose (not the pickup position); the post-drop body is no longer kinematically attached (proven by teleporting the carrier 10m away and confirming the body does not chase); force-drop on knockout-while-carrying severs the attachment; carry survives lit-portal traversal (dossier section 5 edge case 2; `wireTraversal` does not branch on `player.carry` so the state passes through untouched); the per-tick attachment re-anchors the body to the destination spawn pose immediately on the next tick; hard reset clears the carry slot mid-drag. The harness builds a real Rapier world with two real capsule bodies and mirrors the host loop's carry sequence (set linvel, applyCarryAttachment, world.step) in a `tickWhileCarrying` helper. Files: `tests/sim/dragRegression.test.ts`. PR #N.
...
- 2026-05-08: Pickup-and-carry of unconscious body landed (REQ-034 done; section 10 slice 3). New module `src/sim/carryState.ts` exports `PICKUP_RANGE_M = 1.0`, `CARRY_SPEED_MULTIPLIER = 0.6` (Q-005), `CARRY_OFFSET = { x: 0, y: 1.2, z: 0 }`, `INITIAL_CARRY_STATE = { kind: 'idle' }`, the `CarryState` discriminated union (`idle` | `carrying { carriedId }`) and `Carryable` interface, plus three pure helpers: `nearestCarryable` (filters carrier-self and conscious candidates, picks the closest by squared planar distance, breaks ties by smallest id), `applyCarrySpeedScaling` (0.6 multiplier when carrying; identity when idle), and `resolveCarryToggle` (toggle semantics per Q-004 default). New module `src/sim/applyCarry.ts` exports the side-effecting `applyCarryPickup` (kinematic flip + zero linvel), `applyCarryAttachment` (per-tick `setNextKinematicTranslation` to carrier + `CARRY_OFFSET`), `applyCarryDrop` (dynamic flip + snap to floor at carrier's planar position + zero linvel; mesh tilt untouched), `carryTransitionKind` classifier, and `snapCarriedMeshAbove`. `KeyState` gained a `pickup: boolean` channel (`KeyF` default per Q-002). `replayPickupAtTick` is the sibling to `replayPunchAtTick`. `Player` and `ActivePlayerHandle` gained `carry: CarryState`; `Player.setPlanarVelocity` applies `applyCarrySpeedScaling` against the current carry state so every call site automatically gets the slowdown. The host loop in `src/app.ts` derives the pickup rising edge as `keyboard.state.pickup && !previousPickupHeld && consciousness === 'conscious'`, builds a `Carryable[]` from active-timeline ghosts, runs `resolveCarryToggle` (or short-circuits to a forced drop if the carrier was knocked out this tick while carrying), classifies the transition, and applies the side effects on the matching ghost body. `hardReset` returns `player.carry` to `INITIAL_CARRY_STATE`. Q-006 ships only the kinematic flip half (the excluded-collision-group belt is documented as a future tightening). Q-011 default consumed: the carrier's recording captures the pickup flag; the body's trajectory is a deterministic consequence. Ghost carry is NOT enabled this slice (active player only resolves transitions; ghost replay records the flag for a future slice). Files: `src/sim/carryState.ts`, `src/sim/applyCarry.ts`, `src/input/keyboard.ts`, `src/sim/inputRecorder.ts`, `src/scene/player.ts`, `src/sim/portalTraversal.ts`, `src/sim/hardReset.ts`, `src/app.ts`, `tests/sim/carryState.test.ts`, `tests/sim/applyCarry.test.ts`, `tests/sim/inputRecorder.test.ts`, `tests/scene/player.test.ts`, `tests/sim/hardReset.test.ts`. PR #N.
