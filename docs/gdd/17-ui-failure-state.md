...
- 2026-05-14: F-022 pause-menu reset implementation. Added the DOM pause menu, reset confirmation, `Escape` / `R` keyboard routing, touch reset routing through confirmation, and fixed-step pause while the menu is open. Files: `src/render/pauseMenu.ts`, `src/render/actionButtons.ts`, `src/app.ts`, `tests/render/pauseMenu.test.ts`, `docs/GDD_COVERAGE.json`, `docs/FOLLOWUPS.md`, `docs/PROGRESS_LOG.md`. PR pending.
