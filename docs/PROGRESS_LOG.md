...
- PR: (pending)
- Changed: `TimeOfDay` no longer drifts off render-frame delta. Internal state is now an integer `tickIndex` modulo `ticksPerCycle` (= `cycleSeconds * ticksPerSecond`, default 60 * 60 = 3600 ticks per day). New API: `advanceTicks(n)` (integer, throws on non-integer or non-finite, no-op for n <= 0), `tick()` (current tick modulo cycle), and `advanceSeconds(dt)` (rounds to nearest tick, kept as a convenience for callers outside the fixed-step loop). `setNormalized(n)` and `normalized()` continue to interoperate; positions are snapped to whole ticks so the clock is always tick-aligned. `src/app.ts` now calls `timeOfDay.advanceTicks(1)` once per fixed physics step inside the existing 60Hz accumulator loop (PR #4) instead of `timeOfDay.advance(deltaMs / 1000)` once per render frame, so the same number of fixed steps always produces the same `normalized()` output regardless of frame rate. The recorder still captures `timeOfDay.normalized()` per fixed step; it now sees the post-advance value of the tick it is recording, which matches the tick any later instance observing this moment from outside will see (REQ-030 setup). Tests rewritten around the new API and extended with: `advanceTicks(N)` and `N x advanceTicks(1)` produce bit-identical state; advancing exactly `ticksPerCycle * cycles` returns to tick 0 with no drift after 1000 cycles; non-integer / non-finite tick deltas throw; `advanceSeconds(2.5)` lands on tick 150.
...
- Assumptions: clock advances are integer-only at the `advanceTicks` boundary. Float-second callers go through `advanceSeconds`, which rounds. The fixed-step loop calls `advanceTicks(1)` first and then records, so the `timeOfDay` value written into each `InputFrame` is the tick the input was sampled on, not the tick before. This matches what an observing instance from a later loop will see when it walks past this same tick. Default tick rate is 60Hz to match `world.timestep`; `cycleSeconds * ticksPerSecond` must round to a positive integer or the constructor throws (the prototype configuration is 60 * 60 = 3600 exactly, so this is never reached in practice). The integer modulus path uses `% ticksPerCycle` directly; at `ticksPerCycle = 3600` and 60Hz, `tickIndex` will never approach `Number.MAX_SAFE_INTEGER` in any plausible session, but the wrap on every `advanceTicks` call keeps it bounded anyway.
