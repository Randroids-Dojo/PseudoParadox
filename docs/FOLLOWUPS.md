...
- Resolved: PR (pending merge). New `src/render/onboardingOverlay.ts` mounts a top-right DOM hint with a pure `pickOnboardingContent(isCoarsePointer)` helper that returns the keyboard legend plus goal on fine-pointer devices and the goal line only on coarse-pointer devices (where the action buttons already label themselves). The overlay disposes itself on the first `keydown` or `pointerdown` from `window` (one-shot per page life). Wired into `src/app.ts` after the existing overlay setup. Three unit tests cover the content helper.
