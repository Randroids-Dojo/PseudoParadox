...
- Resolved: PR (the solid-walls slice). The dossier's strategy (a) ended up working without touching `PORTAL_TRIGGER_DEPTH` because the math at the trigger zone for a solid wall already accommodates a 0.4-radius capsule pressed against the wall: the capsule center sits inside the existing trigger band at the moment of collision. Solid full-height walls flush with the visual mesh; no door cutouts; no trigger geometry change.
